...
<script type="text/javascript" src="<%=request.getContextPath()%>/Mproject/js/restaurant.js"></script>
...
<%
	...
	RestaurantVO vo = (RestaurantVO)request.getAttribute("restaurantDetail");
...
<script>
	$(function(){
		...
		var options ={
			center : new kakao.maps.LatLng(<%=vo.getRs_lat()%> ,<%=vo.getRs_long()%>),
			level :3
		};
		...
		// 마커가 표시될 위치
		var markerPosition  = new kakao.maps.LatLng(<%=vo.getRs_lat()%> ,<%=vo.getRs_long()%>);
		...
		$('#listBtn').on('click',function(){
			location.href="<%=request.getContextPath()%>/Mproject/jsp/restaurant.jsp";
		});
		...
		$(document).on('click','.comment-delete',function(){
			...
			if(confirm("정말로 삭제하시겠습니까?")){
				location.href="<%=request.getContextPath() %>/deleteCommentRS.do?cmNo="+cmNo+"&&ntNo="+"<%=vo.getRs_no()%>";
			}else{
				return;
			}
		})
		...
		<section id="container">
			<div id="content">
				<article class="area_preview">
					<div class="wrap">
						<div id="hitNum">조회수 : <%=vo.getRs_hit() %></div>
						...
						<!-- 좋아요, 싫어요 -->
						<div class="like_wrap">
							<div class="good_ico gbBtn" id="good_ico"><a href="#"><img src="<%=request.getContextPath()%>/Mproject/images/good.PNG"><span class="good_num" id="good_num"><%=vo.getRs_good() %></span></a> </div>
                     		<div class="bad_ico gbBtn" id="bad_ico"><a href="#"><img src="<%=request.getContextPath()%>/Mproject/images/bad.PNG"><span class="good_num" id="bad_num"><%=vo.getRs_bad() %></span></a> </div>
						</div>
									...
									<div class="inner">
										...
										<div class="comment-input-area">
											...
											<div class="input-wrap">
												...
												<form id="sendComment" action="<%=request.getContextPath()%>/insertRSComment.do" method="post">
													<input type="hidden" name="ntNo" value="<%=vo.getRs_no()%>">
