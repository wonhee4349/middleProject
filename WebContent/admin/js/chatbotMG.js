...
$.chatbotList = function(cpage){
	sword = $('#sword').val();
	$.ajax({
		url : `${mypath}/chatMG.do`,
		type : 'get',
		data : {"sword":sword,
				"cpage":cpage},
		...
		success : function(res){
			...
			code =`<table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'>
                    ...
                    <th width='88%' style='text-align:center;'>질문 내용</th></tr></a></thead>
                 	<tbody>`;
			$.each(res.list, function(i,v){
				code +=`<tr>
							<td style='text-align:center;'>${(i+1)}</td>
							<td ><a href='#' class="chatDetailMG" id=${v.cb_no}>${v.cb_con}</a></td>
						</tr>
						`;
			}); //반복문 끝
