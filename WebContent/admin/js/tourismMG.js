...
$.tourDetail = function(trNo){
	
	$.ajax({
		url : `${mypath}/tourismDetail.do`,
		type : 'get',
		data : "trNo="+trNo,
	...
	$.ajax({
		...
		success : function(res){
			code =`<table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'>
                    ...
                    <th width='88%' style='text-align:center;'>관광지 이름</th></tr></a></thead>
                 	<tbody>`;
			$.each(res.list, function(i,v){
				code +=`
					<tr><td style='text-align:center;'>${(i+1)}</td><td ><a href='#' class="tourDetailMG" id=${v.tr_no}>${v.tr_name}</a></td></tr>
					`;
			}); //반복문 끝
