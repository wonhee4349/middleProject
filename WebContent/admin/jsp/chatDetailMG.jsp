...
<script src="<%=request.getContextPath()%>/admin/js/chatbotMG.js"  type="text/javascript"></script>
<script>

<%

	ChatVO vo = (ChatVO)request.getAttribute("chatDetail");
	String inputId = (String)session.getAttribute("LOGINID");
%>
$(function(){
	$('#deleteBtn').on('click',function(){
		if(confirm("정말로 삭제하시겠습니까?")){
			location.href="<%=request.getContextPath() %>/deleteChat.do?cbNo=<%=vo.getCb_no()%>"
		}else{
			return;
		}
	})
...
%>							
							 ...
							 <span
								class="mr-2 d-none d-lg-inline text-gray-600 small"><%=inputId %>님</span>
<%
	}else{
...
%>  								 
				...
				<div class="container-fluid">
					...
					<!-- Page Heading -->
					<h1 class="h3 mb-2 text-gray-800">챗봇 Q&A</h1>
					<p class="mb-4">챗봇 Q&A 관리 페이지입니다.</p>

					<!-- DataTales Example -->
					<div class="card shadow mb-4">
						<div class="card-header py-3">
							<h6 class="m-0 font-weight-bold text-primary">chatbot Information</h6>
						</div>
						...
						<div class="card-body">
							...
							<div class="table-responsive" id="showdetails">
									<table class="table table-bordered sytable" id="dataTable" cellspacing="0">
										<tbody>
										<tr>
											<th class="syths">질문 번호</th>
											<td class="tds"><%=vo.getCb_no() %></td>								
										</tr>
										<tr>
											<th class="syths">질문 내용</th>
											<td class="tds"><%=vo.getCb_con() %></td>						
										</tr>
										<tr>
											<th class="syths">답변 내용</th>
											<td class="tds"><%=vo.getCb_rp() %></td>							
										</tr>
										<tr>
											<th class="syths">관리자 번호</th>
											<td class="tds"><%=vo.getMg_no() %></td>							
										</tr>
					...
					<a href="<%=request.getContextPath() %>/admin/jsp/chatbotMG.jsp"
						class="btn btn-primary  btn-icon-split  mr10 f-right mb50"> <span
						class="text">목록</span>
					</a>
