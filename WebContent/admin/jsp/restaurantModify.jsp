...
<script>
	$(function(){
		...
		$.restaurantUpdatePage(no);
		
		/* vparents = $(this).parents('.mbody');
		
		vrsno = $(vparents).find('.rsno').text();
		
		vrsname = $(vparents).find('.rsname').text();
		
		vrsaddr = $(vparents).find('.rsaddr').text();
		
		vrstel = $(vparents).find('.rstel').text();
		
		vrslat = $(vparents).find('.rslat').text();
		
		vrslong = $(vparents).find('.rslong').text();
		
		vrscont = $(vparents).find('.rscont').html().trim();
		
		vsrscont = vcont.replace(/<br>/g, ""); */
	...
	$(document).on('click','#updateRS', function(){		
		
		vrsno = $('#mtable #rsno').text().trim();
		vrsname = $('#mtable #rsname').val();
		vrsaddr = $('#mtable #rsaddr').val();
		vrstel = $('#mtable #rstel').val();
		vrslat = $('#mtable #rslat').val();
		vrslong = $('#mtable #rslong').val();
		vrscont = $('#mtable #rscon').val();	
		
		//vrscont = vrscont.replace(/\n/g, "<br>");
		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<!-- Page Heading -->
					<h1 class="h3 mb-2 text-gray-800">맛집정보</h1>
					<p class="mb-4">맛집정보 관리 페이지입니다.</p>
					...
					<div class="card shadow mb-4">
						...
						<div class="card-body">
							...
							<div class="table-responsive" id="modify">
					...
					<button type="button" id="updateRS"
						class="btn btn-primary  btn-icon-split  mr10 f-right mb50 btpd">수정</button>

					<a href="<%=request.getContextPath() %>/admin/jsp/food_board.jsp"
