...
<script src="<%=request.getContextPath()%>/admin/js/usersMG.js"  type="text/javascript"></script>
<script>

<%

	MemberVO vo = (MemberVO)request.getAttribute("usersDetail");
...
$(function(){
	$('#deleteBtn').on('click',function(){
		if(confirm("정말로 삭제하시겠습니까?")){
			location.href="<%=request.getContextPath() %>/deleteUsers.do?usesrNo=<%=vo.getUsers_no()%>"
		}else{
			return;
		}
	})
...
%>
					...
					<ul class="navbar-nav ml-auto">
						...
						<!-- Nav Item - Alerts -->
						<li class="nav-item dropdown no-arrow mx-1"> <!-- Dropdown - Alerts -->
							<div
...
%>                        
				...
				<div class="container-fluid">
						...
						<div class="card-body">
							...
							<div class="table-responsive" id="showdetails">
									<table class="table table-bordered sytable" id="dataTable" cellspacing="0">
										<tbody>
										<tr>
											<th class="syths">사용자 번호</th>
											<td class="tds"><%=vo.getUsers_no() %></td>								
										</tr>
										<tr>
											<th class="syths">이름</th>
											<td class="tds"><%=vo.getUsers_name() %></td>						
										</tr>
										<tr>
											<th class="syths">아이디</th>
											<td class="tds"><%=vo.getUsers_id() %></td>							
										</tr>
										<tr>
											<th class="syths">비밀번호</th>
											<td class="tds"><%=vo.getUsers_pw() %></td>							
										</tr>
										<tr>
											<th class="syths">생일</th>
											<td class="tds"><%=vo.getUsers_bir() %></td>							
										</tr>
										<tr>
											<th class="syths">전화번호</th>
											<td class="tds"><%=vo.getUsers_tel() %></td>					
										</tr>
										<tr>
											<th class="syths">우편번호</th>
											<td class="tds"><%=vo.getUsers_zip() %></td>					
										</tr>
										<tr>
											<th class="syths">이메일</th>
											<td class="tds"><%=vo.getUsers_mail() %></td>					
										</tr>
										<tr>
											<th class="syths">주소</th>
											<td class="tds"><%=vo.getUsers_addr1() %></td>					
										</tr>
										<tr>
											<th class="syths">상세주소</th>
											<td class="tds"><%=vo.getUsers_addr2() %></td>					
										</tr>
										<tr>
											<th class="syths">성별</th>
											<td class="tds"><%=vo.getUsers_gen() %></td>					
										</tr>
										</tbody>
										</table>
							</div>
					...
					<a href="<%=request.getContextPath() %>/admin/jsp/usersMG.jsp"
						class="btn btn-primary  btn-icon-split  mr10 f-right mb50"> <span
						class="text">목록</span>
					</a>
